...
class ShowAllRecentScreen extends StatefulWidget {
  final String title;

  const ShowAllRecentScreen({super.key, required this.title});

  @override
  State<ShowAllRecentScreen> createState() => _ShowAllRecentScreenState();
}

class _ShowAllRecentScreenState extends State<ShowAllRecentScreen> {
  ...
  List<search_model.Datum> _allItems = [];
  List<search_model.Datum> _filteredItems = [];
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      do {
        ...
        final snapshot = getSearchFilterRxObj.dataFetcher.value;
        if (snapshot is search_model.SearchFilterModel) {
      ...
      log("Error fetching recent celebrities: $e");
    } finally {
  ...
  void _applyFilters() {
    ...
    setState(() {
      List<search_model.Datum> result = List.from(_allItems);
