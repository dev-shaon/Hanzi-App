...
class VerfiyScreen extends StatefulWidget {
  final dynamic roleSelected;
  const VerfiyScreen({super.key, required this.roleSelected});
...
class _VerfiyScreenState extends State<VerfiyScreen> {
  ...
  void _resendOtp() async {
    ...
    try {
      await postSignUpGetOtpRx.post(email: appData.read(KKeyEmail).toString());
  ...
  void _verifyOtp() async {
    ...
    try {
      if (widget.roleSelected == 'forget') {
        NavigationService.navigateToReplacementUntil(
          Routes.resetPasswordScreen,
        );
      } else {
        bool success = await postVerifyOtpRx.post(
          otp: _otpController.text.trim(),
          email: appData.read(KKeyEmail).toString(),
        );

        if (success) {
          await appData.remove(KKeyPendingEmail);

          int? role = appData.read(KKeyroleSelected);

          customToastMessage("Success", "Verification successful!");

          if (role == 3) {
            NavigationService.navigateToReplacementUntil(Routes.talentAccount);
          } else if (role == 4) {
