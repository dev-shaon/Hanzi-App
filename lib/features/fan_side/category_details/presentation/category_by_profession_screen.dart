...
class CategoryByProfessionScreen extends StatefulWidget {
  final String title;
  final int id;

  const CategoryByProfessionScreen({
    super.key,
    required this.title,
    required this.id,
  });
  ...
  @override
  State<CategoryByProfessionScreen> createState() =>
      _CategoryByProfessionScreenState();
}

class _CategoryByProfessionScreenState
    extends State<CategoryByProfessionScreen> {
  ...
  // Raw list fetched from the API; used as source for local filtering
  List<cat_model.Celebrity> _allItems = [];
  // The list currently shown (after search / price filter applied)
  List<cat_model.Celebrity> _filteredItems = [];
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      do {
        await getCategoryByProfessionRxObj.fetchCategoryByProfession(
          widget.id,
          page: currentPage,
        );
        final snapshot = getCategoryByProfessionRxObj.dataFetcher.value;
        if (snapshot is cat_model.CategoryByProfessionModel) {
          final newItems = snapshot.data?.celebrities ?? [];
  ...
  void _applyFilters() {
    ...
    setState(() {
      List<cat_model.Celebrity> result = List.from(_allItems);
